...
export default function ForemanCalendar() {
  ...
  const fetchData = async () => {
    try {
      ...
      // ดึงข้อมูลวันลา และส่งข้อมูลครึ่งวัน/ช่วงเวลาเข้า extendedProps
        ...
        data.forEach((lv) => {
          ...
          const baseEvent = {
            ...
            extendedProps: { 
                ...
                totalDays: lv.totalDays || lv.durationDays || '1', // เก็บจำนวนวัน
                leaveTimeSlot: lv.leaveTimeSlot || '' // เก็บช่วงเวลา
            }
          };
          allEvents.push(...splitEventsByWorkDays(baseEvent, holidaySet));
        });
      }
      ...
      setUserOptions([
          { value: "all", label: "👥 ดูลูกน้องทั้งหมด" }, 
          ...uniqueUsersArray
      ]);
  ...
  // ✅ แสดงผล เช้า/บ่าย โดยดึงจากข้อมูลที่ได้รับ
    ...
    <div style={{ padding: isMobile ? 15 : 40, fontFamily: "'Sarabun', sans-serif", background: '#f8fafc', minHeight: '100vh' }}>
      ...
      <div style={{ display: 'flex', flexDirection: isMobile ? 'column' : 'row', justifyContent: 'space-between', alignItems: isMobile ? 'flex-start' : 'center', marginBottom: 25, gap: 15 }}>
        <div>
           ...
           <p style={{ margin: '5px 0 0', color: '#64748b', fontSize: '14px' }}>ตารางวันลาของพนักงานในความดูแล</p>
        </div>
      ...
      <div style={{ background: "#ffffff", padding: isMobile ? 15 : 30, borderRadius: 20, boxShadow: "0 10px 15px -3px rgba(0, 0, 0, 0.05), 0 4px 6px -2px rgba(0, 0, 0, 0.025)", border: '1px solid #f1f5f9' }}>
        <FullCalendar
          ...
          buttonText={{ today: 'วันนี้', month: 'ตารางเดือน', list: 'รายการ' }}
          locale="th"
          ...
          dayCellClassNames={(arg) => {
            ...
            return holidayDateSet.has(dateStr) ? 'holiday-cell' : '';
          }}
        />
      </div>
    </div>
