...
const styles = `
  ...
  /* Style ให้ เสาร์-อาทิตย์ เป็นสีเทา */
  .fc-day-sat, .fc-day-sun {
    background-color: #f8fafc !important; 
  }

  /* Style ใหม่: สำหรับวันหยุดนักขัตฤกษ์ */
  .holiday-cell {
...
export default function AdminCalendar() {
        ...
        data.forEach((lv) => {
          ...
          const baseEvent = {
            ...
            extendedProps: { 
                ...
                // ✅ Update: ดึงข้อมูลจำนวนวันมาตรวจสอบว่าเป็นครึ่งวันหรือไม่
  ...
  const handleDateClick = async (arg) => {
    const { value: holidayName } = await Swal.fire({
      title: 'เพิ่มวันหยุดนักขัตฤกษ์',
      text: `วันที่: ${arg.dateStr}`,
      input: 'text',
      inputPlaceholder: 'ใส่ชื่อวันหยุด (เช่น วันสงกรานต์)',
    ...
    if (holidayName) {
      try {
        await API.post('/holidays', { date: arg.dateStr, name: holidayName });
        Swal.fire('สำเร็จ', 'บันทึกวันหยุดเรียบร้อย', 'success');
        fetchData(); 
      } catch (error) {
        console.error(error);
        Swal.fire('ผิดพลาด', 'ไม่สามารถบันทึกวันหยุดได้', 'error');
      }
    }
  };
  ...
  const handleEventClick = (clickInfo) => {
    ...
    if (category === 'holiday') {
        Swal.fire({
            title: 'ลบวันหยุด?',
            text: `ต้องการลบ "${clickInfo.event.title}" ออกจากปฏิทินหรือไม่?`,
            icon: 'warning',
            showCancelButton: true,
            ...
            if (result.isConfirmed) {
                try {
                    const holidayId = clickInfo.event.id.replace('holiday-', '');
                    await API.delete(`/holidays/${holidayId}`);
                    Swal.fire('ลบสำเร็จ', 'ข้อมูลวันหยุดถูกลบแล้ว', 'success');
                    fetchData(); 
                } catch (err) {
  ...
  // ✅✅ แก้ไขจุดแสดงผล: ใช้ Logic ตรวจสอบจากจำนวนวัน (0.5) แทน
  const renderEventContent = (eventInfo) => {
    const { typeName, category, totalDays, leaveTimeSlot } = eventInfo.event.extendedProps;
    let icon = "";
    if (category === 'holiday') icon = ""; 
    
    const isHoliday = category === 'holiday';
    const isHalfDay = parseFloat(totalDays) === 0.5; // ✅ เช็คจากวัน 0.5 โดยตรง

    // ✅ คำนวณข้อความช่วงเวลา
    let timeLabel = "";
    if (isHalfDay) {
    ...
    <div style={{ padding: isMobile ? 15 : 40, fontFamily: "'Sarabun', sans-serif", background: '#f8fafc', minHeight: '100vh' }}>
      ...
      <div style={{ display: 'flex', flexDirection: isMobile ? 'column' : 'row', justifyContent: 'space-between', alignItems: isMobile ? 'flex-start' : 'center', marginBottom: 25, gap: 15 }}>
        <div>
           ...
           <p style={{ margin: '5px 0 0', color: '#64748b', fontSize: '14px' }}>คลิกที่ช่องวันที่ว่าง เพื่อเพิ่มวันหยุดนักขัตฤกษ์</p>
        </div>
