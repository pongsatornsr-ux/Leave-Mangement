...
const decryptData = (cipherText) => {
    ...
    try {
        ...
        return originalText || cipherText; // ถ้าถอดได้คืนค่า ถ้าไม่ได้คืนค่าเดิม
    } catch (error) {
        return cipherText; // คืนค่าเดิมกรณี Error (เช่น ข้อมูลเก่าไม่ได้เข้ารหัส)
    }
};
    ...
    (async () => {
      ...
      try {
        const results = await Promise.allSettled([
          API.get('/personalleaves/admin'), // แก้ endpoint ให้ตรงกับ backend ของ Admin
        ...
        if (results[0].status === 'fulfilled') {
            ...
            // ✅ 4. ถอดรหัส sickReason ก่อนนำไปรวมใน Array
            const decryptedData = data.map(l => ({
                ...l,
                sourceTable: 'personal',
                // เช็คว่าเป็นลาป่วยหรือไม่ ถ้าใช่ให้ถอดรหัส
